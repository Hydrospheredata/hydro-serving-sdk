...
def test_predict_nparray():
    ...
    predictor_client = created_servable.predictor(return_type=PredictorDT.DICT_NP_ARRAY)
    inputs = {'in1': np.array([1])}
    predictions = predictor_client.predict(inputs=inputs)

    assert isinstance(predictions['out1'], np.ndarray)
    assert isinstance(predictions, dict)
...
def test_predict_df():
    ...
    predictor_client = created_servable.predictor(return_type=PredictorDT.DF)
    inputs_dict = {'in1': [1]}
